...
EXPLICACION = {
    "listas_en_conflicto": "Excel guarda las listas desplegables por rango de filas. Cuando una columna tiene más de una "
    "definición, y no coinciden entre sí, hay que decidir cuál vale.",
    "titulo_duplicado": "Dos columnas distintas con el mismo título. El nombre técnico no puede repetirse dentro de una "
    "hoja, así que hay que desambiguarlos.",
    "lista_huerfana": "La lista está escrita en el archivo pero ninguna columna la usa. Puede ser un olvido (la columna "
    "existe pero quedó sin desplegable) o una lista en desuso.",
    "valor_con_espacios": "El valor tiene espacios al principio o al final. Para una persona es invisible; para el "
    "importador es un valor distinto.",
    "variantes_entre_catalogos": "El mismo texto escrito de dos maneras en listas diferentes del archivo.",
    "catalogos_parecidos": "Dos listas que comparten la mayoría de sus valores pero no todos. Podrían ser la misma lista en "
    "dos versiones, o dos listas legítimamente distintas.",
    "ayuda_sin_campo": "La hoja de instrucciones explica algo que no coincide con el título de ninguna columna. Suele ser "
    "el nombre de un grupo de campos, o un título que cambió.",
    "ayuda_ambigua": "El mismo título aparece en varias columnas y la sección de la hoja de instrucciones no alcanza "
    ...
    "nombre_tecnico_desambiguado": "Dos columnas daban el mismo nombre técnico. Se les agregó el grupo de campos para diferenciarlas.",
    "formato_contradice_al_nombre": "La celda tiene aplicado un formato que no se corresponde con lo que el nombre del campo sugiere. "
    "Suele indicar que la planilla quedó mal formateada. Se propone el tipo según el nombre.",
    "formato_aplicado_en_bloque": "El mismo formato está aplicado a la mayoría de las columnas de la hoja. No puede ser una decisión "
    "sobre cada campo: alguien seleccionó la hoja entera. Se ignora como evidencia del tipo de dato.",
    "obligatoriedad_ambigua": "La validación de esa columna no declara si admite celdas vacías, mientras el resto del archivo sí "
    "lo declara. Se toma como descuido, no como marca de obligatoriedad.",
    "validaciones_rescatadas": "Excel guarda de otra manera las listas que viven en otra hoja del archivo, y la librería estándar "
...
def main():
    p = argparse.ArgumentParser(
        description="Genera el informe de estructura de un archivo."
    )
    p.add_argument("base")
    ...
    for h in mapa["hojas"]:
        w(
            f'| {h["nombre"]} | datos | {len(h["columnas"])} columnas, {len(h["dimensiones"])} grupos |'
        )
    ...
    for h in mapa["hojas"]:
        ...
        w(
            f'Grupos de campos en la fila {h["fila_dimensiones"] or "—"}, títulos de columna en la fila '
        ...
        if h.get("notas"):
            ...
            for nota in h["notas"]:
                w(
                    f'- *{nota["texto"]}* (fila {nota["fila"]}, columna {nota["columna"]})'
                )
            w("")
        if h["filas_con_datos"] == 0 and h["filas_solo_placeholder"] > 0:
            w(
                f'**Es una plantilla vacía:** {h["filas_solo_placeholder"]} filas preparadas, ninguna con datos reales.'
            )
        else:
            w(
                f'Filas con datos: {h["filas_con_datos"]}. Filas sólo con texto de relleno: {h["filas_solo_placeholder"]}.'
            )
        w("")
        if h["dimensiones"]:
            ...
            for d in h["dimensiones"]:
                w(
                    f'| {d["nombre"]} | {d["col_desde"]} a {d["col_hasta"]} | {d["cantidad_columnas"]} |'
                )
        ...
        w("")
        w(
            "| Col | Nombre técnico | Título | Grupo | Tipo | Largo | Oblig. | Confianza |"
        )
        w("|---|---|---|---|---|---|---|---|")
        for c in h["columnas"]:
            cf = c["inferencia"]["tipo"]["confianza"] if c.get("inferencia") else "—"
            w(
                f'| {c["letra"]} | `{c["nombre_tecnico"]}` | {c["titulo"]} | {c["dimension"] or "—"} | '
    ...
    if mapa.get("inferencia"):
        ...
        w("")
        w(
            "El Excel casi nunca dice de qué tipo es cada campo. Lo que sigue son **propuestas**,"
        )
        ...
        w("|---|---|")
        w(
            "| **alta** | Sale de algo que el archivo dice: la lista de valores, el formato de la celda, o los datos cargados. |"
        )
        w(
            "| **media** | El archivo dice una cosa y el nombre del campo otra. Se optó por el nombre. |"
        )
        w(
            "| **baja** | El archivo no dice nada. Se dedujo del nombre del campo, o es el valor por defecto. |"
        )
        w("")
        w(f'Tipos: {" · ".join(f"**{k}** {v}" for k, v in inf["por_tipo"].items())}.')
        w(
            f'Confianza: {" · ".join(f"**{k}** {v}" for k, v in inf["por_confianza"].items())}.'
        )
        w(
            f'Campos propuestos como obligatorios: **{inf["obligatorios"]}**. '
        ...
        for h in mapa["hojas"]:
            no_texto = [
                c
                for c in h["columnas"]
                if c.get("tipo_dato") and c["tipo_dato"] != "TEXTO"
            ]
            if no_texto:
                ...
                for c in no_texto:
                    w(
                        f'| {c["letra"]} | {c["titulo"]} | {c["tipo_dato"]} | {c["inferencia"]["tipo"]["origen"]} |'
                    )
                w("")
            reglas = [
                (c, r) for c in h["columnas"] for r in c.get("reglas_sugeridas", [])
            ]
            if reglas:
                ...
                for c, r in reglas:
                    w(
                        f'| {c["letra"]} | {c["titulo"]} | `{r["tipo_regla"]}` | {r["severidad"]} | '
    ...
    w(
        f'Se encontraron **{len(mapa["catalogos"])} listas distintas**, contando {total_apar} apariciones.'
    )
    w(
        "Dos listas con los mismos valores se consideran la misma, aunque estén escritas en lugares diferentes."
    )
    w("")
    ...
    for c in mapa["catalogos"]:
        ...
        w(
            f'| {n[:70]} | {c["cantidad"]} | {veces} {"vez" if veces == 1 else "veces"} | `{c["huella"]}` |'
        )
    w("")
    w(
        "La **huella** es lo que permite reconocer una lista cuando reaparece en otro archivo."
    )
    w("")
        ...
        if t == "listas_en_conflicto":
            ...
            for an in arr:
                ...
                if an.get("solo_en_el_primero"):
                    w(
                        f'   - sólo en la primera: {", ".join(f"`{x}`" for x in an["solo_en_el_primero"][:10])}'
                    )
                if an.get("solo_en_el_segundo"):
                    w(
                        f'   - sólo en la segunda: {", ".join(f"`{x}`" for x in an["solo_en_el_segundo"][:10])}'
                    )
            w("")
        else:
            for an in arr:
                ...
                pref = (
                    f"**{donde}**{enc}: "
                    if donde
                    else (f'**{an["titulo"]}**: ' if an.get("titulo") else "")
                )
