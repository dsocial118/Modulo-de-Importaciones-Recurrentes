...
PISTAS = [
    (r"\bfecha\b|\bnacimiento\b|\bvencimiento\b|\balta\b|\begreso\b|\bingreso\b", "FECHA", None, None),
    (r"\bcuil\b|\bcuit\b", "TEXTO", 13, "se guarda como texto para no perder ceros ni guiones"),
    (r"\bdni\b|\bdocumento\b.*\bn|\bn[°º]\s*doc", "TEXTO", 15,
     "puede traer puntos, letras o documentos extranjeros"),
    (r"\bedad\b|\bcantidad\b|\bcant\.|\btotal\b|\bnumero de\b|\bnro\.? de\b", "ENTERO", None, None),
    (r"\bcodigo postal\b|\bcp\b", "TEXTO", 10, "los códigos postales argentinos llevan letras"),
    ...
    (r"\bobservacion|\bcomentario|\bdetalle\b|\bdescripcion\b|\bespecificar\b", "TEXTO", None,
     "texto libre y largo"),
    (r"\bprovincia\b|\blocalidad\b|\bpartido\b|\bmunicipio\b|\bpais\b", "TEXTO", 120, None),
]

# Campos que, por su rol, suelen ser imprescindibles para identificar la fila.
IDENTIFICATORIOS = [
    r"\bapellido\b", r"\bnombre/?s?\b", r"\bdni\b", r"\bcuil\b",
    r"\bfecha del relevamiento\b", r"\bfecha de nacimiento\b",
]
...
def _obligatoriedad(col: dict, titulo_clave: str) -> tuple[bool, str, str]:
    ...
    if col.get("no_admite_vacio_explicito"):
        return True, "alta", "la validación de Excel declara explícitamente que no admite celdas vacías"
...
def inferir_campo(col: dict, tiene_catalogo: bool = False, cantidad_valores: int = 0,
                  largo_maximo_valor: int = 0) -> dict:
    ...
    if tiene_catalogo and clase_de_formato(col.get("formato")) in ("fecha", "entero", "decimal"):
        conflicto = {
            "segun_formato": clase_de_formato(col.get("formato")).upper(),
            "segun_nombre": "TEXTO",
            "detalle": (f'La columna tiene formato "{col.get("formato")}" pero su lista de valores '
                        f'admite {cantidad_valores} opciones de texto. El formato está mal aplicado.'),
        }
        return {
            "tipo_dato": "TEXTO",
            "longitud_maxima": max(largo_maximo_valor, 20) if largo_maximo_valor else 255,
            "obligatorio": _obligatoriedad(col, t)[0],
            "conflicto_de_tipo": conflicto,
            "inferencia": {
                "tipo": {"valor": "TEXTO", "confianza": "alta",
                         "origen": f"lista de {cantidad_valores} valores, por encima del formato de celda",
                         "nota": None},
                "longitud": {"valor": largo_maximo_valor or 255, "origen": "medido sobre los valores del catálogo"},
                "obligatorio": dict(zip(("valor", "confianza", "origen"), _obligatoriedad(col, t))),
            },
        }
    ...
    if clase == "fecha":
        tipo, confianza, origen = "FECHA", "alta", f'formato de celda "{col.get("formato")}"'
    elif clase == "decimal":
        tipo, confianza, origen = "DECIMAL", "alta", f'formato de celda "{col.get("formato")}"'
    elif clase == "entero":
        tipo, confianza, origen = "ENTERO", "alta", f'formato de celda "{col.get("formato")}"'
    elif clase == "hora":
    ...
    if tipo and por_nombre and por_nombre["tipo"] != tipo and origen and origen.startswith("formato"):
        conflicto = {
            "segun_formato": tipo,
            "segun_nombre": por_nombre["tipo"],
            "detalle": (f'El formato de celda es "{col.get("formato")}" ({tipo}), pero el campo se llama '
    ...
    return {
        ...
        "conflicto_de_tipo": conflicto,
        "inferencia": {
            "tipo": {"valor": tipo, "confianza": confianza, "origen": origen, "nota": nota},
            "longitud": {"valor": largo,
                         "origen": "no aplica" if largo is None else ("medido sobre los datos" if muestras else "propuesto")},
            "obligatorio": {"valor": obligatorio, "confianza": oblig_confianza, "origen": oblig_origen},
        },
    }
