    ...
    },
    {
        ...
        "parametros": [
            (
                ...
                "CAMPO",
                True,
                "Campo cuyo valor dispara la obligatoriedad.",
            ),
            (
                "operador",
                "TEXTO",
    ...
    },
    {
        ...
        "parametros": [
            (
                ...
                "CAMPO",
                True,
                "Campo del mismo registro contra el que se compara.",
            ),
            (
                "operador",
                "TEXTO",
                True,
                "IGUAL, DISTINTO, MAYOR, MAYOR_IGUAL, MENOR o MENOR_IGUAL.",
            ),
        ],
    },
    {
        ...
        "parametros": [
            ("formato", "TEXTO", True, "Patrón esperado, por ejemplo NN-NNNNNNNN-N.")
        ],
    },
    {
...
DEPENDIENTES = [
    (
        ...
        r"\bse identifica con algun pueblo originario\b",
        "Sí",
    ),
...
def sugerir_reglas(col: dict, todas: list[dict]) -> list[dict]:
    ...
    if re.search(r"\bcuil\b|\bcuit\b", t):
        agregar(
            "EJECUTAR_FUNCION",
            {"funcion": "validar_cuil"},
            "el campo es un CUIL y el apartado técnico ya prevé esa función",
            "alta",
            "BLOQUEANTE",
        )
        agregar(
            ...
            "formato de CUIL indicado como ejemplo en el apartado técnico",
            "media",
            "ADVERTENCIA",
        )
    if re.search(r"\bmail\b|\bcorreo\b|\be-?mail\b", t):
        agregar(
            "EJECUTAR_FUNCION",
            {"funcion": "validar_mail"},
            "el campo es una dirección de correo",
            "alta",
            "ADVERTENCIA",
        )
    if re.search(r"\bdni\b", t) and "referente" not in t:
        agregar(
            "UNICO_EN_HOJA",
            {},
            "el requerimiento pide que en un mismo Excel no haya un DNI repetido",
            "alta",
            "BLOQUEANTE",
        )
    ...
    if re.search(r"\bedad\b", t):
        agregar(
            "RANGO",
            {"minimo": 0, "maximo": 17},
            "el registro es de niños, niñas y adolescentes",
            "baja",
            "ADVERTENCIA",
        )
    if col.get("tipo_dato") == "FECHA":
        agregar(
            "COMPARAR_VALOR",
            {"operador": "MENOR_IGUAL", "valor": "HOY"},
            "una fecha registrada no debería ser futura",
            "media",
            "ADVERTENCIA",
        )
        ...
        for patron_fin, patron_inicio in PARES_FECHA:
            ...
            inicio = next(
                (
                    ...
                    for o in todas
                    if o is not col
                    and o.get("tipo_dato") == "FECHA"
                    and re.search(patron_inicio, clave(o.get("titulo", "")))
                ),
                None,
            )
            if inicio:
                agregar(
                    "COMPARAR_CAMPO",
                    {
                        "campo_comparacion": inicio["nombre_tecnico"],
    ...
    for patron, patron_disp, valor in DEPENDIENTES:
        ...
        disp = next(
            (
                o
                for o in todas
                if o is not col and re.search(patron_disp, clave(o.get("titulo", "")))
            ),
            None,
        )
        if disp:
            agregar(
                "OBLIGATORIO_SI",
                {
                    "campo_condicion": disp["nombre_tecnico"],
                    "operador": "IGUAL",
    ...
    if re.search(r"\bespecificar\b|\botros\b|\bcual\b", t) and not any(
        s["tipo_regla"] == "OBLIGATORIO_SI" for s in sug
    ):
        previo = next(
            (
                o
                for o in todas
                if o.get("orden") == col.get("orden", 0) - 1
                and o.get("catalogo_huella")
            ),
            None,
        )
        if previo:
            agregar(
                "OBLIGATORIO_SI",
                {
                    "campo_condicion": previo["nombre_tecnico"],
                    "operador": "EN_LISTA",
                    "valor_condicion": "Otros",
                },
                f'es el campo de detalle de "{previo["titulo"]}"',
                "baja",
                "ADVERTENCIA",
            )
...
def sugerir_reglas_de_hoja(hoja: dict) -> list[dict]:
    ...
    dni = next(
        (
            c
            for c in cols
            if re.search(r"\bdni\b", clave(c["titulo"]))
            and "referente" not in clave(c["titulo"])
        ),
        None,
    )
    fecha = next(
        (
            c
            for c in cols
            if c.get("tipo_dato") == "FECHA"
            and re.search(r"\bmedida\b|\bingreso\b|\binicio\b", clave(c["titulo"]))
        ),
        None,
    )
    ...
    return [
        {
            ...
            "parametros": {
                "campos_combinados": [dni["nombre_tecnico"], fecha["nombre_tecnico"]]
            },
