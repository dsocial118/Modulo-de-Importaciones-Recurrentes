    ...
    if ya:
        ...
        for v in cat["valores"]:
            if v not in conocidos:
                ya["opciones"].append({
                    "codigo": codificar(v), "valor_esperado": v,
                    "orden": len(ya["opciones"]) + 1,
                    "nuevo_en": date.today().isoformat(),
                })
        return ya

    base_cod = codificar(cat["nombres_vistos"][0] if cat["nombres_vistos"] else "catalogo")
...
def main():
    ...
    w(f'-- Generado el {datetime.now().isoformat(timespec="seconds").replace("T", " ")} a partir de:')
    ...
    for t in TIPOS_REGLA:
        w(f'INSERT INTO runac_c1_tipo_regla (nombre, descripcion) VALUES ({q(t["nombre"])}, {q(t["descripcion"])})')
        w("  ON DUPLICATE KEY UPDATE descripcion = VALUES(descripcion);")
        for i, (nom, tipo_par, oblig, desc) in enumerate(t["parametros"], start=1):
            w("INSERT INTO runac_c1_tipo_regla_parametro (tipo_regla_id, nombre, tipo_parametro, obligatorio, orden, descripcion)")
            w(f"  SELECT id, {q(nom)}, {q(tipo_par)}, {1 if oblig else 0}, {i}, {q(desc)}")
            w(f'    FROM runac_c1_tipo_regla WHERE nombre = {q(t["nombre"])}')
            w("  ON DUPLICATE KEY UPDATE descripcion = VALUES(descripcion), tipo_parametro = VALUES(tipo_parametro);")
    w("")
    ...
    for cat in mapa["catalogos"]:
        ...
        desc = f'Huella {cat["huella"]}. Aparece como: {" / ".join(d["nombres_vistos"])}.'
        w(f'-- {d["codigo"]}: {len(d["opciones"])} opciones')
        w("INSERT INTO runac_c1_catalogo (codigo, nombre, descripcion) VALUES "
          f'({q(d["codigo"])}, {q(d["nombre"][:255])}, {q(desc)})')
        w("  ON DUPLICATE KEY UPDATE nombre = VALUES(nombre), descripcion = VALUES(descripcion);")
        for o in d["opciones"]:
            w("INSERT INTO runac_c1_catalogo_opcion (catalogo_id, codigo, valor_esperado, orden, activo)")
            w(f'  SELECT id, {q(o["codigo"])}, {q(o["valor_esperado"][:255])}, {o["orden"]}, 1 '
              f'FROM runac_c1_catalogo WHERE codigo = {q(d["codigo"])}')
            w("  ON DUPLICATE KEY UPDATE valor_esperado = VALUES(valor_esperado), orden = VALUES(orden), activo = 1;")
    w("")
    ...
    w(f"-- Archivo {codigo}")
    w("-- ============================================================")
    w("-- El archivo se actualiza en lugar de borrarse: la Capa 2 lo referencia")
    w("-- desde runac_c2_estructura y no se puede dejar esa referencia colgando.")
    w("INSERT INTO runac_c1_archivo (codigo, nombre_esperado, titulo, subtitulo, descripcion, orden_importacion, obligatorio)")
    w(f'VALUES ({q(codigo)}, {q(mapa["archivo"]["nombre_fisico"])}, {q(titulo)}, {q(subtitulo)}, '
      f"{q(desc_archivo)}, {args.orden}, 1)")
    w("ON DUPLICATE KEY UPDATE nombre_esperado = VALUES(nombre_esperado), titulo = VALUES(titulo), "
      "subtitulo = VALUES(subtitulo), descripcion = VALUES(descripcion), orden_importacion = VALUES(orden_importacion);")
    w(f"SET @archivo := (SELECT id FROM runac_c1_archivo WHERE codigo = {q(codigo)});")
    w("")
    w("-- Su contenido sí se rehace entero, de abajo hacia arriba para no violar")
    w("-- las claves foráneas.")
    w("DELETE cr FROM runac_c1_campo_regla cr")
    w("  JOIN runac_c1_campo c ON c.id = cr.campo_id")
    w("  JOIN runac_c1_hoja h ON h.id = c.hoja_id WHERE h.archivo_id = @archivo;")
    w("DELETE c FROM runac_c1_campo c JOIN runac_c1_hoja h ON h.id = c.hoja_id WHERE h.archivo_id = @archivo;")
    w("DELETE d FROM runac_c1_dimension d JOIN runac_c1_hoja h ON h.id = d.hoja_id WHERE h.archivo_id = @archivo;")
    w("DELETE FROM runac_c1_hoja WHERE archivo_id = @archivo;")
    w("")

    n_reglas = n_campos = 0
    for i_hoja, h in enumerate(mapa["hojas"], start=1):
        w(f'-- --- hoja "{h["nombre"]}" ---')
        w("INSERT INTO runac_c1_hoja (archivo_id, nombre_esperado, descripcion, orden_procesamiento, "
          "fila_encabezados, obligatoria)")
        w(f'VALUES (@archivo, {q(h["nombre"])}, {q(h.get("titulo_general"))}, {i_hoja}, '
          f'{h["fila_encabezados"]}, 1);')
        w("SET @hoja := LAST_INSERT_ID();")
        w("")

        for d in h["dimensiones"]:
            desc_dim = (f'Columnas {d["col_desde"]} a {d["col_hasta"]} del Excel '
                        f'({d["cantidad_columnas"]} campos).')
            w("INSERT INTO runac_c1_dimension (hoja_id, nombre_esperado, descripcion, orden)")
            w(f'VALUES (@hoja, {q(d["nombre"])}, {q(desc_dim)}, {d["orden"]});')
        w("")

        for col in h["columnas"]:
            n_campos += 1
            dim_sub = (f'(SELECT id FROM runac_c1_dimension WHERE hoja_id = @hoja AND nombre_esperado = {q(col["dimension"])})'
                       if col.get("dimension") else "NULL")
            cat = cat_por_huella.get(col.get("catalogo_huella")) if col.get("catalogo_huella") else None
            cat_sub = f'(SELECT id FROM runac_c1_catalogo WHERE codigo = {q(cat["codigo"])})' if cat else "NULL"
            inf = col["inferencia"]

            w(f'-- {col["letra"]}: {col["titulo"]}')
            w(f'--    tipo {col["tipo_dato"]} por {inf["tipo"]["origen"]} (confianza {inf["tipo"]["confianza"]})')
            if col.get("ayuda_origen"):
                w(f'--    ayuda tomada de {col["ayuda_origen"]}')
            w("INSERT INTO runac_c1_campo (hoja_id, dimension_id, catalogo_id, nombre, titulo_esperado, orden, "
              "tipo_dato, longitud_maxima, obligatorio, ayuda)")
            w(f'VALUES (@hoja, {dim_sub}, {cat_sub}, {q(col["nombre_tecnico"])}, {q(col["titulo"][:255])}, '
            ...
            for r in col.get("reglas_sugeridas", []):
                n_reglas += 1
                nombre_regla = f'{codigo}_{col["nombre_tecnico"]}_{r["tipo_regla"]}'.lower()[:255]
                desc_regla = f'{r["motivo"]} (confianza {r["confianza"]})'
                w("INSERT INTO runac_c1_regla (tipo_regla_id, nombre, descripcion, parametros)")
                w(f'  SELECT id, {q(nombre_regla)}, {q(desc_regla)}, '
                  f'{q(json.dumps(r["parametros"], ensure_ascii=False))}')
                w(f'    FROM runac_c1_tipo_regla WHERE nombre = {q(r["tipo_regla"])}')
                w("  ON DUPLICATE KEY UPDATE descripcion = VALUES(descripcion), parametros = VALUES(parametros);")
                w("INSERT INTO runac_c1_campo_regla (campo_id, regla_id, severidad)")
                w(f'  SELECT c.id, r.id, {q(r["severidad"])}')
                w("    FROM runac_c1_campo c, runac_c1_regla r")
                w(f'   WHERE c.hoja_id = @hoja AND c.nombre = {q(col["nombre_tecnico"])} '
    ...
    w("COMMIT;")
    w("")
    w(f'-- Resumen: {len(mapa["hojas"])} hoja(s), {n_campos} campos, '
    ...
    dec["arch"]["origen"] = {"nombre_fisico": mapa["archivo"]["nombre_fisico"], "sha1": mapa["archivo"]["sha1"]}
    for h in mapa["hojas"]:
        for col in h["columnas"]:
            prev = dec["arch"]["campos"].get(col["nombre_tecnico"], {"titulos_vistos": []})
            if col["titulo"] not in prev["titulos_vistos"]:
                prev["titulos_vistos"].append(col["titulo"])
            prev.update({
                "hoja": h["nombre"], "letra": col["letra"], "orden": col["orden"],
                "tipo_dato": col["tipo_dato"], "obligatorio": col["obligatorio"],
                "catalogo": cat_por_huella[col["catalogo_huella"]]["codigo"] if col.get("catalogo_huella") else None,
            })
            dec["arch"]["campos"][col["nombre_tecnico"]] = prev
    guardar_decisiones(dec)

    print(f"sql: {destino}")
    print(f'  {len(mapa["hojas"])} hoja(s), {n_campos} campos, {len(mapa["catalogos"])} catalogos, {n_reglas} reglas')
