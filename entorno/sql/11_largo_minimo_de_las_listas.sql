...
--
--   la definición   mir_c1_campo.longitud_maxima
...
UPDATE `mir_c1_campo` c
  JOIN (
    SELECT cat.id, MAX(CHAR_LENGTH(o.valor_esperado)) AS mas_largo
      FROM `mir_c1_catalogo` cat
      JOIN `mir_c1_catalogo_opcion` o ON o.catalogo_id = cat.id
     GROUP BY cat.id
...
ALTER TABLE `mir_c2_mpe_v1` MODIFY `modalidad_de_cuidado` varchar(22) NULL;
